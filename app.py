...
# MongoDB setup
client = MongoClient("mongodb+srv://lekya:lekhya123@cluster0.dnmvp.mongodb.net/")
...
# Flask app setup
api = Flask(__name__)  # Corrected _name_ to __name__
CORS(api)
...
def store_sensor_data():
    """Store sensor data (MAX30102, DHT11, Turbidity, Gas) in MongoDB."""
    try:
        data = request.json
        ...
        # Handle Turbidity sensor type
        elif sensor_type == "turbidity":
            turbidity_value = sensor_data.get("turbidity")
            if turbidity_value is None:
                return jsonify({"error": "Missing 'turbidity' for turbidity sensor"}), 400
        ...
        # Handle Gas sensor type
        elif sensor_type == "gas":
            gas_concentration = sensor_data.get("gas_concentration")
            if gas_concentration is None:
                return jsonify({"error": "Missing 'gas_concentration' for gas sensor"}), 400
